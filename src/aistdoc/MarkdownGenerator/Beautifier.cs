    ...
    internal static class Beautifier {
        ...
        public static string ToMarkdownMethodInfo(MethodInfo methodInfo) {
            ...
            var seq = methodInfo.GetParameters().Select(x => {
                var suffix = x.HasDefaultValue ? (" = " + (x.DefaultValue ?? $"<span style='color: blue'>null</span>")) : "";
                return "`" + BeautifyType(x.ParameterType) + "` " + x.Name + suffix;
            });

            return methodInfo.Name + "(" + (isExtension ? "<span style='color: blue'>this</span> " : "") + string.Join(", ", seq) + ")";
        }
    }
}
